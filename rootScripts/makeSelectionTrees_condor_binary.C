...
*/
  ...
  for (int i=0; i<chainentries; i++)
      ...
      for (const auto & muon : *preselected_muons_intree)
	{
	  if (muon.validFrac >= 0.8 && muon.lepMVA > 0.75) {
	     ...
	     if (muon.segCompatibility > segCom) numEntries += 1;
      ...
      for (const auto & elec : *preselected_electrons_intree) {
         ...
         if (elec.lepMVA > 0.75) numEntries += 1;
      }
  ...
  cout << "pass Common: " << passCommonCount << endl;
...
void makeSelectionTrees_small(char** argv)
{
