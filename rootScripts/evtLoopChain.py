#!/usr/bin/env python

import os
import time

import ROOT
ROOT.gSystem.Load('CMSSW_7_6_3/lib/slc6_amd64_gcc493/libttH-13TeVMultiLeptonsTemplateMakers.so')

startTime = time.time()

#chain for ttH
ch_ttH = ROOT.TChain('OSTwoLepAna/summaryTree')
for rootFile in os.listdir('/hadoop/users/klannon/ttHTrees/ttH125powheg/'):
   ch_ttH.Add('/hadoop/users/klannon/ttHTrees/ttH125powheg/'+rootFile)

#chain for lep from T
ch_ttJetT = ROOT.TChain('OSTwoLepAna/summaryTree')
for rootFile in os.listdir('/hadoop/users/klannon/ttHTrees/TTJets_split_1lepFromT'):
   ch_ttJetT.Add('/hadoop/users/klannon/ttHTrees/TTJets_split_1lepFromT/'+rootFile)
   
#chain for lep from TBar
ch_ttJetTBar = ROOT.TChain('OSTwoLepAna/summaryTree')
for rootFile in os.listdir('/hadoop/users/klannon/ttHTrees/TTJets_split_1lepFromTBar'):
   ch_ttJetTBar.Add('/hadoop/users/klannon/ttHTrees/TTJets_split_1lepFromTBar/'+rootFile)

print('chains done')
chainTime = time.time() - startTime

#scale factor
ttbarCross = 182.7
ttHCross = 0.212
luminosity = 2500
nTotSig = 3991615 #signalTree.GetEntries()
nTotBar = 61614751 #backgroundTree.GetEntries()
nTotT = 61374841

scaleFactorH = luminosity*ttHCross/nTotSig
scaleFactorT = luminosity*ttbarCross/nTotT
scaleFactorBar = luminosity*ttbarCross/nTotBar

#define the styles
sigStyle = ROOT.TStyle('sigStyle','sigStyle')
sigStyle.SetHistFillColor(ROOT.kBlue)
backStyle = ROOT.TStyle('backStyle', 'backStyle')
backStyle.SetHistFillColor(ROOT.kRed)

#set style
ROOT.gROOT.SetStyle('sigStyle')

#define cuts
#cut = entry.tightMvaBased_leptons.size()==2&&entry.tightMvaBased_leptons[0].charge*entry.tightMvaBased_leptons[1].charge>0&&entry.preselected_jets.size()>2

#start timing for histograms
startHistTime = time.time()

#add background histograms under signal histogram
#also add THStack hists
lepPtHistSig = ROOT.TH1F('lepPtHistSig','Lepton p_{T}',100,0,200)
lepPtStack = ROOT.THStack('lepPtStack','lepton p_{T}')

lepEtaHistSig = ROOT.TH1F('lepEtaHistSig','Lepton #eta',100,-2,2)
lepEtaStack = ROOT.THStack('lepEtaStack', 'Lepton #eta')

nJetsHistSig = ROOT.TH1F('nJetsHistSig','N_{Jets}',10,0,10)
nJetsStack = ROOT.THStack('nJetsStack','N_{Jets}')

nJets50HistSig = ROOT.TH1F('nJets50HistSig','N_{Jets}',10,0,10)
nJets50Stack = ROOT.THStack('nJets50Stack','N_{Jets}')

#added by Matthew Link
lepPtHistSig2 = ROOT.TH1F('lepPtHistSig2', 'Lepton p_{T}2',100,0,200)
lepPtStack2 = ROOT.THStack('lepPtStack2', 'Lepton p_{T}2')

lepPxHistSig = ROOT.TH1F('lepPxHistSig', 'Lepton p_{X}', 100,0,200)
lepPxStack = ROOT.THStack('lepPxStack','Lepton p_{X}')

lepRhoHistSig = ROOT.TH1F('lepRhoHistSig', 'Lepton #rho', 100,0,10)
lepRhoStack = ROOT.THStack('lepRhoStack', 'Lepton #rho')

jetChargeHistSig = ROOT.TH1F('jetChargeHistSig', 'Jet Charge', 100, -2,2)
jetChargeStack = ROOT.THStack('jetChargeStack', 'Jet Charge')

#have not created stack plots for charge2 or csv
jetChargeHistSig2 = ROOT.TH1F('jetChargeHistSig2', 'Jet Charge Pt>42',100,-2,2)
jetCSVHistSig = ROOT.TH1F('jetCSVHistSig', 'Jet csv',100,-10,2)

#loop for ttH
for entry in ch_ttH:
   if entry.tightMvaBased_leptons.size()==2 and entry.tightMvaBased_leptons[0].charge*entry.tightMvaBased_leptons[1].charge>0 and entry.preselected_jets.size()>2:

	    # Fill the lepton plots
	    for lep in entry.preselected_leptons:
		lepPtHistSig.Fill(lep.obj.Pt())
	
		lepPxHistSig.Fill(lep.obj.Px())
		lepRhoHistSig.Fill(lep.rho)

		if lep.obj.Pt() > 35:
		    lepEtaHistSig.Fill(lep.obj.Eta())

		# Not in drawTree. testing more complicate if
		if lep.obj.Px() + lep.obj.Py() > 42:
		    lepPtHistSig2.Fill(lep.obj.Pt())
	


	    # Fill jet plots
	    nJet = 0
	    nJet50 = 0
	    for jet in entry.preselected_jets:

		nJet += 1
		if jet.obj.Pt() > 50:
		    nJet50 += 1
	
		jetChargeHistSig.Fill(jet.charge)

		if jet.obj.Pt() > 42:
		    jetChargeHistSig2.Fill(jet.charge)

		jetCSVHistSig.Fill(jet.csv)

	    nJetsHistSig.Fill(nJet)
	    nJets50HistSig.Fill(nJet50)
        
#Set background style
ROOT.gROOT.SetStyle('backStyle')

#create background hists
lepPtHistBack = ROOT.TH1F('lepPtHistBack','Lepton p_{T} Background',100,0,200)
lepPtHistBackBar = ROOT.TH1F('lepPtHistBackBar','Lepton p_{T} Background',100,0,200)

lepEtaHistBack = ROOT.TH1F('lepEtaHistBack','Lepton #eta Background',100,-2,2)
lepEtaHistBackBar = ROOT.TH1F('lepEtaHistBackBar','Lepton #eta Background',100,-2,2)

nJetsHistBack = ROOT.TH1F('nJetsHistBack','N_{Jets} back',10,0,10)
nJetsHistBackBar = ROOT.TH1F('nJetsHistBackBar','N_{Jets} back',10,0,10)

nJets50HistBack = ROOT.TH1F('nJets50HistBack','N_{Jets} back',10,0,10)
nJets50HistBackBar = ROOT.TH1F('nJets50HistBackBar','N_{Jets} back',10,0,10)

lepPtHistBack2 = ROOT.TH1F('lepPtHistBack2','Lepton p_{T}2',100,0,200)
lepPtHistBack2Bar = ROOT.TH1F('lepPtHistBack2Bar','Lepton p_{T}2',100,0,200)

lepPxHistBack = ROOT.TH1F('lepPxHistBack','Lepton p_{X}',100,0,200)
lepPxHistBackBar = ROOT.TH1F('lepPxHistBackBar','Lepton p_{X}',100,0,200)

lepRhoHistBack = ROOT.TH1F('lepRhoHistBack', 'Lepton #rho',100,0,10)
lepRhoHistBackBar = ROOT.TH1F('lepRhoHistBackBar', 'Lepton #rho',100,0,10)

jetChargeHistBack = ROOT.TH1F('jetChargeHistBack','Jet Charge',100,-2,2)
jetChargeHistBackBar = ROOT.TH1F('jetChargeHistBackBar','Jet Charge',100,-2,2)
   
#Fill the background plots
for entry in ch_ttJetT:
   if entry.tightMvaBased_leptons.size()==2 and entry.tightMvaBased_leptons[0].charge*entry.tightMvaBased_leptons[1].charge>0 and entry.preselected_jets.size()>2:

	    # Fill the lepton plots
	    for lep in entry.preselected_leptons:
		lepPtHistBack.Fill(lep.obj.Pt()) 
	
		lepPxHistBack.Fill(lep.obj.Px())
		lepRhoHistBack.Fill(lep.rho)

		if lep.obj.Pt() > 35:
		    lepEtaHistBack.Fill(lep.obj.Eta()) 
	
		if lep.obj.Px() + lep.obj.Py() > 42:
		    lepPtHistBack2.Fill(lep.obj.Pt())
	
	    # Fill jet plots
	    nJet = 0
	    nJet50 = 0
	    for jet in entry.preselected_jets:

		nJet += 1
		if jet.obj.Pt() > 50:
		    nJet50 += 1  
	
		jetChargeHistBack.Fill(jet.charge)
		 
	    nJetsHistBack.Fill(nJet)
	    nJets50HistBack.Fill(nJet50)

for entry in ch_ttJetTBar:
   if entry.tightMvaBased_leptons.size()==2 and entry.tightMvaBased_leptons[0].charge*entry.tightMvaBased_leptons[1].charge>0 and entry.preselected_jets.size()>2:
	    # Fill the lepton plots
	    for lep in entry.preselected_leptons:
		lepPtHistBackBar.Fill(lep.obj.Pt()) 
	
		lepPxHistBackBar.Fill(lep.obj.Px())
		lepRhoHistBackBar.Fill(lep.rho)

		if lep.obj.Pt() > 35:
		    lepEtaHistBackBar.Fill(lep.obj.Eta()) 
	
		if lep.obj.Px() + lep.obj.Py() > 42:
		    lepPtHistBack2Bar.Fill(lep.obj.Pt())
	
	    # Fill jet plots
	    nJet = 0
	    nJet50 = 0
	    for jet in entry.preselected_jets:

		nJet += 1
		if jet.obj.Pt() > 50:
		    nJet50 += 1  
	
		jetChargeHistBackBar.Fill(jet.charge)
		 
	    nJetsHistBackBar.Fill(nJet)
	    nJets50HistBackBar.Fill(nJet50)

#scale the signals and background
lepPtHistSig.Scale(scaleFactorH)
lepPtHistBack.Scale(scaleFactorT)
lepPtHistBackBar.Scale(scaleFactorBar)


lepEtaHistSig.Scale(scaleFactorH)
lepEtaHistBack.Scale(scaleFactorT)
lepEtaHistBackBar.Scale(scaleFactorBar)

nJetsHistSig.Scale(scaleFactorH)
nJetsHistBack.Scale(scaleFactorT)
nJetsHistBackBar.Scale(scaleFactorBar)

nJets50HistSig.Scale(scaleFactorH)
nJets50HistBack.Scale(scaleFactorT)
nJets50HistBackBar.Scale(scaleFactorBar)

lepPtHistSig2.Scale(scaleFactorH)
lepPtHistBack2.Scale(scaleFactorT)
lepPtHistBack2Bar.Scale(scaleFactorBar)

lepPxHistSig.Scale(scaleFactorH)
lepPxHistBack.Scale(scaleFactorT)
lepPxHistBackBar.Scale(scaleFactorBar)

lepRhoHistSig.Scale(scaleFactorH)
lepRhoHistBack.Scale(scaleFactorT)
lepRhoHistBackBar.Scale(scaleFactorBar)

jetChargeHistSig.Scale(scaleFactorH)
jetChargeHistBack.Scale(scaleFactorT)
jetChargeHistBackBar.Scale(scaleFactorBar)

#total background signals
lepPtHistBack.Add(lepPtHistBack,lepPtHistBackBar)
lepEtaHistBack.Add(lepEtaHistBack,lepEtaHistBackBar)
nJetsHistBack.Add(nJetsHistBack,nJetsHistBackBar)
nJets50HistBack.Add(nJets50HistBack,nJets50HistBackBar)
lepPtHistBack2.Add(lepPtHistBack2,lepPtHistBack2Bar)
lepPxHistBack.Add(lepPxHistBack,lepPxHistBackBar)
lepRhoHistBack.Add(lepRhoHistBack,lepRhoHistBackBar)
jetChargeHistBack.Add(jetChargeHistBack,jetChargeHistBackBar)


#end time for histograms
endHistTime = time.time() - startHistTime

#Fill THStack - make sure back is 1st
lepPtStack.Add(lepPtHistBack)
lepPtStack.Add(lepPtHistSig)

lepEtaStack.Add(lepEtaHistBack)
lepEtaStack.Add(lepEtaHistSig)

nJetsStack.Add(nJetsHistBack)
nJetsStack.Add(nJetsHistSig)

nJets50Stack.Add(nJets50HistBack)
nJets50Stack.Add(nJets50HistSig)

lepPtStack2.Add(lepPtHistBack2)
lepPtStack2.Add(lepPtHistSig2)

lepPxStack.Add(lepPxHistBack)
lepPxStack.Add(lepPxHistSig)

lepRhoStack.Add(lepRhoHistBack)
lepRhoStack.Add(lepRhoHistSig)

jetChargeStack.Add(jetChargeHistBack)
jetChargeStack.Add(jetChargeHistSig)


outfile = ROOT.TFile.Open('evtLoopChain_output.root','RECREATE')
outfile.cd()

#write the signals
lepPtHistSig.Write()
lepEtaHistSig.Write()
nJetsHistSig.Write()
nJets50HistSig.Write()
lepPtHistSig2.Write()
lepPxHistSig.Write()
lepRhoHistSig.Write()
jetChargeHistSig.Write()
jetChargeHistSig2.Write()
jetCSVHistSig.Write()

#write the backgrounds
lepPtHistBack.Write()
lepEtaHistBack.Write()
nJets50HistBack.Write()
nJetsHistBack.Write()
lepPtHistBack2.Write()
lepPxHistBack.Write()
lepRhoHistBack.Write()
jetChargeHistBack.Write()

#write the THStack
lepPtStack.Write()
lepEtaStack.Write()
nJetsStack.Write()
nJets50Stack.Write()
lepPtStack2.Write()
lepPxStack.Write()
lepRhoStack.Write()
jetChargeStack.Write()

totalTime = time.time() - startTime
print('drawChain\n fill chain: {}\nhistograms: {}\nTotal: {}'.format( chainTime, endHistTime, totalTime))

outfile.Close()


