...
/////////////////////////////////////////
///
/// usage: root -l makeSelectionTrees_small.C+
///
/////////////////////////////////////////
...
void run_it(TChain* chain, TString output_file)
{
  ...
  TTree *object_selection_tree = (TTree*)chain->CloneTree(0);
  ...
  for (int i=0; i<chainentries; i++)
      ...
      if (i%10000 == 0)
	{
      ...
      //create vectors for storing tight muons/electrons
      //vector<ttH::Electron> tightElec;
      //vector<ttH::Muon> tightMu;
      ...
      for (const auto & muon : *preselected_muons_intree)
	{
      ...
      for (const auto & elec : *preselected_electrons_intree) {
         ...
         if (elec.lepMVA > 0.75 && elec.numMissingInnerHits == 0 && elec.passConversioVeto){
	    numEntries += 1;
      ...
      //if (passesCommon )passCommonCount += 1;
  ...
  cout << "pass Common: " << passCommonCount << " tightMu: " << numTightMu << " tightElec: " << numTightElec << endl;
...
/*
  std::ifstream infile("files_forSelection.txt");
  TString line;
  while (infile >> line ){
     chain->Add(line );
     cout << line << endl;
