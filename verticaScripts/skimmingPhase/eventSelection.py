...
cur.execute("CREATE TABLE tightMVABased_Electrons AS SELECT *, CASE WHEN (lepMVA > 0.75 ) THEN 1 ELSE 0 END FROM preselected_electrons")

cur.execute("CREATE TABLE tightMVABased_Muons AS SELECT * FROM preselected_muons WHERE validFrac >= 0.8 and lepMVA > 0.75 and segCompatibility > CASE WHEN isGlobalMuon = 1 and normalizedChi2 < 3 and localChi2 < 12 and trKink < 20 THEN 0.303 ELSE 0.451 END")
